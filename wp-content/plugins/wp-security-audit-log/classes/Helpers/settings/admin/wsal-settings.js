...
$doc.ready(function () {
  ...
  var wsalBlocksColorsOptions = {
    change: function (event, ui) {
      ...
      jQuery(this)
        .closest('.block-item')
        .find('.wsal-block-head')
        .attr('style', 'background-color: ' + newColor)
        .removeClass('block-head-light block-head-dark')
        .addClass('block-head-' + getContrastColor(newColor));
    },
    clear: function () {
      jQuery(this)
        .closest('.block-item')
  ...
  jQuery('#wsal_form').submit(function (evt) {
    ...
    jQuery('form#wsal_form input:disabled, form#wsal_form textarea:disabled, form#wsal_form select:disabled').attr(
      'disabled',
      false,
    );
    ...
    jQuery.ajax({
      ...
      error: function (xhr, status, error) {
        if ('undefined' != typeof xhr.status && xhr.status != 200) {
          ...
          $saveAlert.append(
            '<div class="wsal-error-message">' +
              xhr.responseJSON.data[0].message +
              '<p><button id="wsal_remove_error" type="button">Close</button></p></div>',
          );
  ...
  /* SETTINGS PANEL
    ------------------------------------------------------------------------------------------ */
  jQuery('.wsal-panel, .wsal-notice').css({
    opacity: 1,
    visibility: 'visible',
  });
  ...
  if (
    jQuery(currentTab).parent('#wsal_form').length ||
    jQuery(currentTab).parent('#periodic-report-viewer').length ||
    jQuery(currentTab).parent('#saved-reports-viewer').length
  ) {
...
function wsal_color_picker() {
  ...
  jQuery('.wsalColorSelector').each(function () {
    ...
    jQuery(
      '<div class="wsal-alpha-container"><div class="slider-alpha"></div><div class="transparency"></div></div>',
