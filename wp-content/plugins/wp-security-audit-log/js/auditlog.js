      ...
      if (jQuery(this).attr('data-disabled')) {
        ...
        jQuery(this).darkTooltip({
          ...
          onYes: function (elem, tooltip) {
            ...
            var { __ } = wp.i18n;
            jQuery('#darktooltip-' + elem.attr('id') + ' strong').text(
              __('This event is disabled.', 'wp-security-audit-log'),
            );
        ...
        function (data) {
          ...
          if (data && data.length) {
            ...
            for (var i = 0; i < data.length; i++) {
              var link = jQuery('<a href="javascript:;" onclick="WsalSsasChange(' + data[i].blog_id + ')"/>').text(
                data[i].blogname + ' (' + data[i].domain + ')',
              );
          ...
          SsasDiv.prepend(
            jQuery('<a href="javascript:;" onclick="WsalSsasChange(0)" class="allsites"/>').text(
              WsalData.tr8n.searchback,
            ),
          );
          SsasDiv.show();
        },
...
jQuery(document).ready(function () {
   ...
   */
  jQuery('.wsal-addon-install-trigger').click(function () {
    var button = this;
    wsal_addon_installer_ajax(button);
    // disable this button.
    jQuery('.wsal-addon-install-trigger').attr('disabled', true);
    jQuery('.wsal-addon-install-trigger:not([data-addon-name="' + jQuery(this).data('addon-name') + '"])').text(
      wsalAuditLogArgs.installAddonStrings.otherInstalling,
    );
  ...
  jQuery.ajax({
    ...
    success: function (data) {
      ...
      if (data.success === 'undefined' || data.success === false) {
        ...
        jQuery('.wsal-addon-install-trigger[data-addon-name="' + jQuery(button).data('addon-name') + '"]').text(
          wsalAuditLogArgs.installAddonStrings.addonInstalled,
        );
        jQuery(
          '.wsal-addon-install-trigger:not([data-addon-name="' + jQuery(button).data('addon-name') + '"])',
        ).attr('disabled', false);
...
jQuery(document).ready(function () {
  ...
  jQuery(document).on('click', '.data-event-inspector-link:not(.inspector-active)', function (event) {
    ...
    jQuery(this)
      .closest('tr')
      .after(
        '<div class="event-data-panel panel-' + count + '"></div><div class="panel-' + count + '-fill"></div>',
      );
