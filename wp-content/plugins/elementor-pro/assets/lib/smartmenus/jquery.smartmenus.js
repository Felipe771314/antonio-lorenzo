...
(function (factory) {
    ...
    prototype: {
      ...
      menuPosition: function ($sub) {
        var $a = $sub.dataSM('parent-a'),
          ...
          horizontalParent =
            $ul.parent().is('[data-sm-horizontal-sub]') || (level == 2 && !$ul.hasClass('sm-vertical')),
          rightToLeft =
            ...
            if (/(start|down)$/i.test(e.type)) {
              ...
              if (data.touchY !== undefined) {
                if (
                  (data.momentum =
                    Math.pow(
                      Math.abs(touchPoint.pageY - data.touchStartY) / (e.timeStamp - data.touchStartTime),
                      2,
                    ) * 15)
                ) {
                  this.menuScrollStop($sub);
  ...
  $.fn.smartmenus = function (options) {
    ...
    return this.each(function () {
      ...
      if (dataOpts) {
        $.each(
          ['showFunction', 'hideFunction', 'collapsibleShowFunction', 'collapsibleHideFunction'],
          function () {
            if (this in dataOpts) {
