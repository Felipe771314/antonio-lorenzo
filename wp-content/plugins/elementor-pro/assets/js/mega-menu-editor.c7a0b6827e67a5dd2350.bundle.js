...
(self['webpackChunkelementor_pro'] = self['webpackChunkelementor_pro'] || []).push([
        ...
        module.exports = ElementEditorModule.extend({
          ...
          onInit() {
            elementor.channels.editor.on(
              'editor:widget:mega-menu:section_layout:activated',
              this.maybeSetContentWidthValue,
            );
          },
  ...
  \*******************************************************/
      /***/ (__unused_webpack_module, exports, __webpack_require__) => {
        ...
        var _nestedModule = _interopRequireDefault(
          __webpack_require__(/*! ./nested-module */ '../modules/mega-menu/assets/js/editor/nested-module.js'),
        );
        var _editorModule = _interopRequireDefault(
          __webpack_require__(/*! ./editor-module */ '../modules/mega-menu/assets/js/editor/editor-module.js'),
        );
  ...
  \**************************************************************/
      /***/ (__unused_webpack_module, exports, __webpack_require__) => {
        ...
        var _view = _interopRequireDefault(
          __webpack_require__(/*! ./views/view */ '../modules/mega-menu/assets/js/editor/views/view.js'),
        );
        ...
        class View extends $e.components.get('nested-elements').exports.NestedView {
          ...
          onAddChild(childView) {
            const widgetNumber =
              ...
              tabId =
                childView._parent.$el.find(`.e-n-menu-item-title[data-tab-index="${index}"]`)?.attr('id') ||
          ...
          attachBuffer(compositeView, buffer) {
            ...
            if (this.model?.config?.support_improved_repeaters && this.model?.config?.is_interlaced) {
              const childContainerSelector =
                  this.model?.config?.defaults?.child_container_placeholder_selector || '',
          ...
          renderDataBindings(settings, dataBindings, widget = []) {
            ...
            const renderDataBinding = async (dataBinding) => {
              ...
              if (
                this.isInternalUrl(settings?.changed?.__dynamic__?.item_link) &&
                this.isSettingChanged(settings, this.itemLink)
              ) {
              ...
              if (this.isAtomicDynamic(settings.changed, dataBinding, changedControl)) {
                const dynamicValue = await this.getDynamicValue(
                  settings,
                  changedControl,
                  bindingSetting,
                  dataBinding,
                ...
                case 'repeater-item':
                  {
                    ...
                    if (
                      container?.parent?.children.indexOf(container) + 1 ===
                      parseInt(dataBinding.dataset.bindingIndex)
                    ) {
          ...
          isAtomicDynamic(changedSettings, dataBinding, changedControl) {
            return (
              ...
              (this.itemTitle === changedControl || this.itemLink === changedControl)
            );
          }
          async getDynamicValue(settings, changedControlKey, bindingSetting, dataBinding, widget) {
            const dynamicSettings = {
              ...
              valueToParse = this.extractValueToParse(
                this.getChangedData(settings, changedControlKey, bindingSetting),
              );
            ...
            if (undefined !== data) {
              this.tryFormatDynamicMegaMenuUrl(
                valueToParse,
                dataBinding,
                widget,
                changedControlKey,
          ...
          getChangedDynamicControlKey(settings) {
            ...
            const changedControlKey = this.findUniqueKey(
              settings?.changed?.__dynamic__,
              settings?._previousAttributes?.__dynamic__,
            )[0];
          ...
          tryFormatDynamicMegaMenuUrl(valueToParse, dataBinding, widget, changedControl, dynamicSettings) {
            ...
            const value = elementor.dynamicTags.parseTagsText(
              valueToParse,
              dynamicSettings,
            ...
            dataBinding.el =
              Array.from(widget)[0].querySelectorAll('.e-n-menu-title-text')[dataBinding.dataset.bindingIndex - 1];
          }
          ...
          async getDataFromCacheOrBackend(valueToParse, dynamicSettings) {
            try {
              return elementor.dynamicTags.parseTagsText(
                valueToParse,
                dynamicSettings,
                elementor.dynamicTags.getTagDataContent,
              );
              ...
              return !_.isEmpty(elementor.dynamicTags.cache)
                ? elementor.dynamicTags.parseTagsText(
                    valueToParse,
                    dynamicSettings,
                    elementor.dynamicTags.getTagDataContent,
                  )
          ...
          getChangedDataForRemovedItem(settings, changedControlKey, bindingSetting) {
            return (
              settings.attributes?.[changedControlKey]?.[bindingSetting] ||
              settings.attributes?.[changedControlKey]
            );
          }
          ...
          getChangedDataForAddedItem(settings, changedControlKey, bindingSetting) {
            return (
              settings.attributes?.__dynamic__?.[changedControlKey]?.[bindingSetting] ||
              settings.attributes?.__dynamic__?.[changedControlKey]
            );
          }

          // TODO: delete in 3.27.0
          getChangedData(settings, changedControlKey, bindingSetting) {
            const changedDataForRemovedItem = this.getChangedDataForRemovedItem(
                settings,
                changedControlKey,
                bindingSetting,
              ),
              changedDataForAddedItem = this.getChangedDataForAddedItem(
                settings,
                changedControlKey,
                bindingSetting,
              );
          ...
          getRepeaterItemActiveIndex() {
            return (
              this.getContainer().renderer.view.model.changed.editSettings.changed.activeItemIndex ||
          ...
          isSettingChanged(settings, bindingSettings) {
            return (
              settings.attributes.__dynamic__?.[bindingSettings] !==
              settings._previousAttributes.__dynamic__?.[bindingSettings]
            );
          }
          isTitleOrLinkChanged(settings) {
            return (
              this.isSettingChanged(settings, this.itemTitle) || this.isSettingChanged(settings, this.itemLink)
            );
          }
        }
