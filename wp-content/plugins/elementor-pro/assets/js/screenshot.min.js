  ...
  class Screenshot extends elementorModules.ViewModule {
    ...
    getDefaultElements() {
      const e = jQuery(ElementorScreenshotConfig.selector),
        t = e.find('.elementor-section-wrap > .elementor-section, .elementor > .elementor-section');
      return {
        $elementor: e,
        $sections: t,
        $firstSection: t.first(),
        $notElementorElements: elementorCommon.elements.$body.find('> *:not(style, link)').not(e),
        $head: jQuery('head'),
      };
    }
    onInit() {
    ...
    loadExternalCss() {
      const e = [this.getSettings('home_url'), ...this.getSettings('excluded_external_css_urls')]
        .map((e) => `[href^="${e}"]`)
        .join(', ');
      jQuery('link')
        .not(e)
        .each((e, t) => {
          const s = jQuery(t),
            n = s.clone();
          n.attr('href', this.getScreenshotProxyUrl(s.attr('href'))), this.elements.$head.append(n), s.remove();
        });
    }
    loadExternalImages() {
    ...
    removeUnnecessaryElements() {
      ...
      this.elements.$sections
        .filter((t, s) => {
          let n = !1;
          return e >= this.getSettings('crop.height') && (n = !0), (e += jQuery(s).outerHeight()), n;
        })
        .each((e, t) => {
          t.remove();
        }),
