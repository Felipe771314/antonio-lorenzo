  ...
  \********************************************************************************/
      /***/ (__unused_webpack_module, exports, __webpack_require__) => {
        ...
        var _focusableElementSelectors = __webpack_require__(
          /*! elementor-pro/frontend/utils/focusable-element-selectors */ '../assets/dev/js/frontend/utils/focusable-element-selectors.js',
        );
        class SearchKeyboardHandler extends elementorModules.frontend.handlers.Base {
          ...
          loadResultElementsEvents() {
            this.setFocusableElements(
              this.$element[0].querySelector(this.getSettings('selectors.resultsContainer')),
            );
          ...
          areResultsClosed() {
            const existingResults = this.elements.resultsContainer.querySelectorAll(
              this.getSettings('selectors.loopItem'),
            );
  ...
  \***************************************************************/
      /***/ (__unused_webpack_module, exports, __webpack_require__) => {
        ...
        class Search extends elementorModules.frontend.handlers.Base {
          ...
          reInitLinks() {
            this.elements.links = document.querySelectorAll(
              `[data-id="${this.getID()}"] a.page-numbers:not(.current)`,
            );
            this.linksEventListeners();
          }
          onClick() {
          ...
          fetchUpdatedSearchWidgetMarkup() {
            return fetch(
              `${elementorProFrontend.config.urls.rest}elementor-pro/v1/refresh-search`,
              this.getFetchArgumentsForSearchUpdate(),
            );
          }
          ...
          renderLiveResults() {
            ...
            this.elements.resultsContainer.classList.remove('hidden');
            return this.fetchUpdatedSearchWidgetMarkup()
              .then((response) => {
          ...
          hideOtherResults() {
            ...
            const visibleResultsContainers = document.querySelectorAll(
              `${this.getSettings('selectors').resultsContainer}:not(.hidden)`,
            );
            Array.from(visibleResultsContainers)
              .filter((resultsContainer) => !resultsContainer.closest(`.elementor-element-${this.getID()}`))
              .forEach((resultsContainer) => resultsContainer.classList.add('hidden'));
          }
          ...
          updateInputStyle() {
            let iconSlugs =
              arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : ['searchIcon', 'clearIcon'];
          ...
          toggleSearchResultsVisibility(event) {
            const selectors = this.getSettings('selectors'),
              ...
              { target } = event,
              isTargetPartOfResults =
                !!target?.closest(`${widgetWrapper} ${selectors.resultsContainer}`) ||
                (target?.classList?.contains(selectors.resultsContainer) && !!target?.closest(widgetWrapper)),
