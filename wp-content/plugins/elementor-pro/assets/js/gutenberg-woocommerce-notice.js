      ...
      const GutenbergWooCommerceEditorBlockNotice = {
        ...
        async maybeCreateNotice() {
          ...
          const woocommerceCartBlockNames = ['woocommerce/cart', 'woocommerce/classic-shortcode'],
            hasWoocommerceCartBlock =
              undefined !==
              Object.keys(blockNames).find((index) => woocommerceCartBlockNames.includes(blockNames[index]));
        ...
        createNotice() {
          const noticeText = __(
              'This page uses the WooCommerce Cart Block. Before editing the page with Elementor and using the Elementor Cart widget, please check out this article.',
              'elementor-pro',
            ),
        ...
        getCartNoticeDismissalState() {
          return fetch(
            `${elementorCommon.config.urls.rest}elementor-pro/v1/get-notice-dismissed/${this.getPostId()}`,
            {
              method: 'GET',
              headers: {
        ...
        maybeRemoveEmptyDefaultBlockFromBlockNames(blocks) {
          const isBlockDefaultEmptyParagraph =
            1 === blocks.length && 'core/paragraph' === blocks[0].name && '' === blocks[0].attributes.content;
